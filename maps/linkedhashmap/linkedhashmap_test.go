...
import (
	...
	"github.com/monitor1379/yagods/maps/linkedhashmap"
)
...
func TestMapMap(t *testing.T) {
	...
	m.Put("a", 1)
	m.Put("b", 2)
	mappedMap := m.Map(func(key1 string, value1 int) (string, int) {
...
func assertSerialization(m *linkedhashmap.Map[string, string], txt string, t *testing.T) {
...
func benchmarkGet(b *testing.B, m *linkedhashmap.Map[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Get(n)
		}
...
func benchmarkPut(b *testing.B, m *linkedhashmap.Map[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Put(n, struct{}{})
		}
...
func benchmarkRemove(b *testing.B, m *linkedhashmap.Map[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Remove(n)
		}
...
func BenchmarkTreeMapGet100(b *testing.B) {
	b.StopTimer()
	size := 100
	m := linkedhashmap.New[int, struct{}]()
	for n := 0; n < size; n++ {
		m.Put(n, struct{}{})
	}
	b.StartTimer()
