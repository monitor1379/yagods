...
func TestListNew(t *testing.T) {
	...
	if actualValue, ok := list2.Get(1); actualValue != 1379 || !ok {
		t.Errorf("Got %v expected %v", actualValue, 1379)
	}
...
func TestMapFunc(t *testing.T) {
	...
	list.Add(1, 2, 3)
	mappedList := arraylist.Map(list, func(index int, value int) string {
...
func benchmarkGet(b *testing.B, list *arraylist.List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Get(n)
		}
...
func benchmarkAdd(b *testing.B, list *arraylist.List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Add(n)
		}
...
func benchmarkRemove(b *testing.B, list *arraylist.List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Remove(n)
		}
