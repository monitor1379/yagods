...
func TestListNew(t *testing.T) {
	list1 := doublylinkedlist.New[string]()

	if actualValue := list1.Empty(); actualValue != true {
		t.Errorf("Got %v expected %v", actualValue, true)
	}

	list2 := doublylinkedlist.New[string]("1", "b")
...
func TestListValues(t *testing.T) {
	...
	list.Add("a")
	list.Add("b", "c")
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s", list.InterfaceValues()...), "abc"; actualValue != expectedValue {
...
func TestListInsert(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", list.InterfaceValues()...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
}
...
func TestListSet(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s", list.InterfaceValues()...), "abbc"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	list.Set(2, "cc") // last to first traversal
	list.Set(0, "aa") // first to last traversal
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s", list.InterfaceValues()...), "aabbcc"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
}
...
func TestMapFunc(t *testing.T) {
	...
	list.Add(1, 2, 3)
	mappedList := doublylinkedlist.Map(list, func(index int, value int) string {
		return fmt.Sprintf("mapped: %v", value)
	})
	if actualValue, _ := mappedList.Get(0); actualValue != "mapped: 1" {
		t.Errorf("Got %v expected %v", actualValue, "mapped: 1")
	}
	if actualValue, _ := mappedList.Get(1); actualValue != "mapped: 2" {
		t.Errorf("Got %v expected %v", actualValue, "mapped: 2")
	}
	if actualValue, _ := mappedList.Get(2); actualValue != "mapped: 3" {
		t.Errorf("Got %v expected %v", actualValue, "mapped: 3")
	}
	if mappedList.Size() != 3 {
		t.Errorf("Got %v expected %v", mappedList.Size(), 3)
	}
...
func TestListFind(t *testing.T) {
	list := doublylinkedlist.New[string]()
	list.Add("a", "b", "c")
	foundIndex, foundValue, found := list.Find(func(index int, value string) bool {
		return value == "c"
	})
	if foundValue != "c" || foundIndex != 2 || !found {
		t.Errorf("Got %v at %v expected %v at %v", foundValue, foundIndex, "c", 2)
	}
	foundIndex, foundValue, found = list.Find(func(index int, value string) bool {
		return value == "x"
	})
	if foundValue != "" || foundIndex != -1 || found {
...
func benchmarkGet(b *testing.B, list *doublylinkedlist.List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Get(n)
		}
...
func benchmarkAdd(b *testing.B, list *doublylinkedlist.List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Add(n)
		}
...
func benchmarkRemove(b *testing.B, list *doublylinkedlist.List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Remove(n)
		}
